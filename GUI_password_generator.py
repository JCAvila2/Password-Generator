...
# Functions
def generate_random_char(characters):
...
def generate_without_input_characters(password_letters, password_numbers):
...
def generate_password(pool_of_characters, amount_of_letters, amount_of_numbers):
    ...
    if pool_of_characters == "Random":
        print(generate_without_input_characters(int(amount_of_letters), int(amount_of_numbers)))
...
window.mainloop()
"""
pool = input("password test: ")
pool = list(pool)
password = ""
for i in range(len(pool)):
    char = generate_random_char(pool)
    pool.remove(char)
    password += char
print(password)
"""